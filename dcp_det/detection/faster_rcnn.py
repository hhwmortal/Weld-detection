...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        images, targets = self.transform(images, targets)  # 对图像进行预处理，做三件事resize（缩放），标准化，把不同大小图片拼成batch，让不同大小图片可以一起训练
        ...
        detections, detector_losses = self.roi_heads(
            ...
            transformed_images=images.tensors)
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        # 若anchor生成器为空，则自动生成针对resnet50_fpn的anchor生成器，anchor就是预设锚框，不同大小+不同长宽比
        ...
        # 生成RPN通过滑动窗口预测网络部分，主要做两件事1.分类（有没有目标）2.回归（调整框）
        if rpn_head is None:
            rpn_head = RPNHead(
        ...
        # 定义整个RPN框架，RPN就是一个小检测器，包含anchor生成，预测，NMS，采样
        ...
        #  Multi-scale RoIAlign pooling，把不同大小的候选框变成统一大小
        ...
        # 在box_head的输出上预测部分，输出类别概率，bbox偏移
        if box_predictor is None:
            representation_size = 1024
        ...
        # 将roi pooling, box_head以及box_predictor结合在一起，包含roi裁剪，分类，回归，nms
        roi_heads = RoIHeads(
            # box
            ...
            dcp_lambda=dcp_lambda,
            dcp_eps=dcp_eps,
            prior_kernel_size=prior_kernel_size,
            image_mean=image_mean,
            image_std=image_std)  # 0.05  0.5  100
